...
showRank = () => {
  $.get( '/Portrait/Author/showByHeat', ( data ) => {
    ...
    for ( let i = 0; i < list.length && i < 15; i++ ) {
      ...
      const affiliation = $( '<td class="uk-text-truncate"><a href=\''+"/affiliationDetail?affiliationId="+list[ i ].affiliation_id+'\' id=\'' + list[ i ].affiliation_id + '\'>' + list[ i ].affiliation + '</a></td>' )
